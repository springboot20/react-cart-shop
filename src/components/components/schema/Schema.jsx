...
export const basicSchema = yup.object().shape({
  username: yup.string().matches(nameRule, { message: 'only allow letters, number, and underscore' }).required('first name is required'),
  email: yup.string().email('Invalid email address').required('email is required'),
  password: yup.string().matches(passwordRule, { message: 'Password must be at least 8 digits characters long and contain at least one uppercase letter, one lowercase letter, and one number ' }).required('password is required'),
});
